...
## Strategic Context (the project review — scientific consensus)
...
multi-step induction (deferred to  Phase 6b).
-/
...
/-- The certified descent theorem: if the certificate passes all checks
    ...
    (b) By the multi-step stability theorem ( Phase 6b). -/
...
/-!
## Step 9: Assembly — certified_descent_for_class ( Phase 4)
...
/-- **Version 2 — Strong descent (conditional on threshold ≤ residue).**
    ...
    If additionally the threshold is ≤ the residue (Conjecture N0),
...
### Formalization strategy (per the project review):
...
### Caveat (per the project review an earlier finding):
...
/-- Check that the threshold is ≤ the residue (N0 property). -/
...
-- Verify N0 for our example certificates
