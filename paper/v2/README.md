...
Companion materials for the paper *On the non-existence of non-trivial Collatz cycles: a conditional formal proof with documented structural obstructions*, by Eric Merle.
...
| `ProjetCollatz/` | Lean 4 formalization (40 files, 13,719 lines, Mathlib v4.27.0) |
| `probes/` | axiom-profile probes used by the verification script |
| `expected_axioms.md` | reference axiom profile for the central chain |
| `reproduce.sh` | end-to-end verification script |
...
Defined in `ProjetCollatz/Phase58PorteDeuxFinal.lean`. The full axiom dependency can be inspected with `#print axioms ProjetCollatz.no_nontrivial_cycle_final`. The three structure parameters represent:
...
- **`BarinaVerification`** — the computational verification of D. Barina, *Improved verification limit for the convergence of the Collatz conjecture*, *J. Supercomp.* 81 (2025), 810 (extending the verified bound to `2⁷¹`).
...
Approximate runtime, measured on Mac M1 Pro 16 GB:
...
Creative Commons Attribution 4.0 International (CC-BY 4.0), for both the paper sources and the Lean formalization.
...
The BibTeX key is disambiguated to avoid collisions with any future Collatz-related publication by the same author.
